...
type
  ...
  {-- TBoldImageCom --}
  TBoldImageCom = class(TCustomControl, IBoldOCLComponentCom)
  private
    fBoldProperties: TBoldViewerFollowerControllerCom;
    ...
    procedure SetBoldProperties(Value: TBoldViewerFollowerControllerCom);
    procedure SetBoldHandle(value: TBoldElementHandleCom);
    function GetViewer: TBoldAbstractViewAdapterCom;
    procedure SetViewer(Value: TBoldAbstractViewAdapterCom);
  ...
  public
    ...
    property Viewer: TBoldAbstractViewAdapterCom read GetViewer write SetViewer;
  ...
  published
    ...
    property BoldProperties: TBoldViewerFollowerControllerCom read FBoldProperties write SetBoldProperties;
...
constructor TBoldImageCom.Create(AOwner: TComponent);
begin
  ...
  fBoldProperties := TBoldViewerFollowerControllerCom.Create(Self);
...
destructor TBoldImageCom.Destroy;
begin
...
procedure TBoldImageCom.CreateParams(var Params: TCreateParams);
begin
...
procedure TBoldImageCom.SetBoldProperties(Value: TBoldViewerFollowerControllerCom);
begin
  FBoldProperties.Assign(Value);
end;

procedure TBoldImageCom.SetBoldHandle(Value: TBoldElementHandleCom);
begin
    fHandleFollower.BoldHandle := value;
end;

function TBoldImageCom.GetViewer: TBoldAbstractViewAdapterCom;
begin
...
procedure TBoldImageCom.SetAutoSize(Value: Boolean);
begin
...
procedure TBoldImageCom.SetDrawFocus(Value: Boolean);
begin
...
function TBoldImageCom.GetScale: Integer;
begin
  Result := Round(fScale*100);
end;

procedure TBoldImageCom.SetScale(Value: Integer);
begin
...
procedure TBoldImageCom.SetStretchMode(Value: TBoldStretchMode);
begin
...
procedure TBoldImageCom.CMEnter(var Message: TCMEnter);
begin
  if fDrawFocus then
...
procedure TBoldImageCom.CMExit(var Message: TCMExit);
begin
  try
...
procedure TBoldImageCom.WMLButtonDown(var Message: TWMLButtonDown);
begin
  if TabStop and CanFocus then SetFocus;
  inherited;
end;

procedure TBoldImageCom.WMCut(var Message: TMessage);
begin
  CutToClipboard;
end;

procedure TBoldImageCom.WMCopy(var Message: TMessage);
begin
  CopyToClipboard;
end;

procedure TBoldImageCom.WMPaste(var Message: TMessage);
begin
  PasteFromClipboard;
end;

procedure TBoldImageCom.WMSize(var Message: TMessage);
begin
  inherited;
...
{}
function TBoldImageCom.GetPalette: HPALETTE;
begin
...
procedure TBoldImageCom.KeyDown(var Key: Word; Shift: TShiftState);
begin
...
procedure TBoldImageCom.KeyPress(var Key: Char);
begin
...
procedure TBoldImageCom.CalcDisplayRect;
var
  ScaleX,
  ScaleY: Double;
  CR: TRect;
...
procedure TBoldImageCom.AfterMakeUptoDate(Follower: TBoldFollowerCom);
begin
  Invalidate;
end;

function TBoldImageCom.EffectiveReadOnly: Boolean;
begin
  Result := fReadOnly or not fBoldProperties.MayModify(Follower);
end;

procedure TBoldImageCom.CopyToClipboard;
begin
  if Assigned(Viewer) then
    Viewer.CopyToClipboard;
end;

procedure TBoldImageCom.CutToClipboard;
begin
  if Assigned(Viewer) then
  begin
    Viewer.CopyToClipboard;
...
procedure TBoldImageCom.PasteFromClipboard;
var
  ...
  begin
    i := 0;
    while (i < TBoldAbstractViewAdapterCom.ViewAdapterClassCount) and
      not TBoldAbstractViewAdapterCom.GetViewAdapterClass(i).CanPasteFromClipboard(fContentTypeOnPaste) do
        inc(i);
    if i < TBoldAbstractViewAdapterCom.ViewAdapterClassCount then
...
Procedure TBoldImageCom.LoadFromFile(FileName: String);
var
  aViewer: TBoldAbstractViewAdapterCom;
  ...
  begin
    i := 0;
    While (i < TBoldAbstractViewAdapterCom.ViewAdapterClassCount) and
      not TBoldAbstractViewAdapterCom.GetViewAdapterClass(i).CanLoadFromFile(FileName) do
        inc(i);
    if i < TBoldAbstractViewAdapterCom.ViewAdapterClassCount then
      result := TBoldAbstractViewAdapterCom.GetViewAdapterClass(i).Create
    else
...
function TBoldImageCom.GetBoldHandle: TBoldElementHandleCom;
begin
  Result := fHandleFollower.BoldHandle;
end;

function TBoldImageCom.GetFollower: TBoldFollowerCom;
begin
    Result := fHandleFollower.Follower;
end;

function TBoldImageCom.GetContextType: IBoldElementTypeInfo;
begin
...
function TBoldImageCom.GetExpression: String;
begin
  Result := BoldProperties.Expression;
end;

procedure TBoldImageCom.SetExpression(Expression: String);
begin
  BoldProperties.Expression := Expression;
end;

function TBoldImageCom.GetVariableList: IBoldExternalVariableList;
begin
