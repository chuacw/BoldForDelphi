...
unit DerivedAttrExampleClasses;

{$DEFINE DerivedAttrExampleClasses_unitheader}
{$INCLUDE DerivedAttrExampleClasses_Interface.inc}
...
function TProduct._Get_M_retail_Price: TBACurrency;
begin
  assert(ValidateMember('TProduct', 'retail_Price', 2, TBACurrency));
  Result := TBACurrency(BoldMembers[2]);
end;

function TProduct._Getretail_Price: Currency;
begin
  Result := M_retail_Price.AsCurrency;
end;
...
initialization
  CodeDescriptor := GeneratedCodes.AddGeneratedCodeDescriptorWithFunc('DerivedAttrExampleClasses', InstallBusinessClasses, InstallObjectListClasses, GeneratedCodeCRC);
finalization
