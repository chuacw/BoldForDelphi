...
function TDiscount._Get_M_percent: TBAInteger;
begin
  assert(ValidateMember('TDiscount', 'percent', 0, TBAInteger));
  Result := TBAInteger(BoldMembers[0]);
end;

function TDiscount._Getpercent: Integer;
begin
  Result := M_percent.AsInteger;
end;

procedure TDiscount._Setpercent(NewValue: Integer);
begin
  M_percent.AsInteger := NewValue;
end;

function TDiscount._GetOrder: TOrderList;
begin
  assert(ValidateMember('TDiscount', 'Order', 1, TOrderList));
  Result := TOrderList(BoldMembers[1]);
end;

procedure TDiscountList.Add(NewObject: TDiscount);
begin
  if Assigned(NewObject) then
    AddElement(NewObject);
end;

function TDiscountList.IndexOf(anObject: TDiscount): Integer;
begin
  result := IndexOfElement(anObject);
end;

function TDiscountList.Includes(anObject: TDiscount) : Boolean;
begin
  result := IncludesElement(anObject);
end;

function TDiscountList.AddNew: TDiscount;
begin
  result := TDiscount(InternalAddNew);
end;

procedure TDiscountList.Insert(index: Integer; NewObject: TDiscount);
begin
  if assigned(NewObject) then
    InsertElement(index, NewObject);
end;

function TDiscountList.GetBoldObject(index: Integer): TDiscount;
begin
  result := TDiscount(GetElement(index));
end;

procedure TDiscountList.SetBoldObject(index: Integer; NewObject: TDiscount);
begin;
  SetElement(index, NewObject);
end;
...
function TOrder._Get_M_theDiscount: TBoldObjectReference;
begin
  assert(ValidateMember('TOrder', 'theDiscount', 5, TBoldObjectReference));
  Result := TBoldObjectReference(BoldMembers[5]);
end;

function TOrder._GettheDiscount: TDiscount;
begin
  assert(not assigned(M_theDiscount.BoldObject) or (M_theDiscount.BoldObject is TDiscount), SysUtils.format(BoldMemberAssertInvalidObjectType, [ClassName, 'theDiscount', M_theDiscount.BoldObject.ClassName, 'TDiscount']));
  Result := TDiscount(M_theDiscount.BoldObject);
end;

procedure TOrder._SettheDiscount(value: TDiscount);
begin
...
begin
  assert(ValidateMember('TPerson', 'Order', 3, TOrderList));
...
function TProduct._Get_M_CostEuro: TBACurrency;
begin
  assert(ValidateMember('TProduct', 'CostEuro', 1, TBACurrency));
  Result := TBACurrency(BoldMembers[1]);
end;

function TProduct._GetCostEuro: Currency;
begin
  Result := M_CostEuro.AsCurrency;
end;

procedure TProduct._SetCostEuro(NewValue: Currency);
begin
...
begin
  assert(ValidateMember('TProduct', 'description', 2, TBAString));
  Result := TBAString(BoldMembers[2]);
end;
...
begin
  assert(ValidateMember('TProduct', 'OrderItem', 3, TOrderItemList));
  Result := TOrderItemList(BoldMembers[3]);
end;
...
begin
  ...
  BoldObjectListClasses.AddObjectEntry('Discount', TDiscountList);
...
begin
  ...
  BoldObjectClasses.AddObjectEntry('Discount', TDiscount);
