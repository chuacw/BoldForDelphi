...
begin
  if dmMain.BoldExternalDataPersistenceHandle.HasNewObjects then
  begin
    Memo1.Lines.Add(Format('%d new objects added', [dmMain.BoldExternalDataPersistenceHandle.NewObjects.Count]));
    dmMain.BoldExternalDataPersistenceHandle.NewObjects.Clear;
  end;
  if dmMain.BoldExternalDataPersistenceHandle.HasDeletedObjects then
  begin
    Memo1.Lines.Add(Format('%d objects deleted in external database', [dmMain.BoldExternalDataPersistenceHandle.DeletedObjects.Count]));
    dmMain.BoldExternalDataPersistenceHandle.DeletedObjects.Clear;
  end;
end;
